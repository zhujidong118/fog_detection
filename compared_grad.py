...
def load_data():
    ...
    x_data = np.array([[table.cell_value(i, 3)] for i in range(row_num)])

    return x_data, y_label

def draw_compared_grad(x_data, y_label):
    ...
    ax.legend(loc="best")
    fig.suptitle("Fog Level After AG")
    plt.show()

if __name__ == '__main__':
    x_data, y_data = load_data()
    draw_compared_grad(x_data, y_data)